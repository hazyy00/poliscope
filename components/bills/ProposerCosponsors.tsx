...
import type { Cosponsor } from './CosponsorsModal'
...
interface Props {
  proposer: Proposer
  proposerNames: string | null
  cosponsors: Cosponsor[]
  cosponsorCount: number
}

export function ProposerCosponsors({ proposer, proposerNames, cosponsors, cosponsorCount }: Props) {
  ...
  const displayNames = proposerNames
    ? proposerNames.split(',').map((s: string) => s.trim()).join(' · ')
    : proposer.name

  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      <Link
        href={`/members/${proposer.id}`}
      ...
      {cosponsorCount > 0 && (
        <>
          <button
            ...
            style={{
              fontSize: 12, color: open ? 'var(--t1)' : 'var(--t3)',
          ...
          {open && pos && (
            ...
            >
              <div style={{
                ...
                공동발의 {cosponsorCount}명
              </div>
              <div style={{ overflowY: 'auto', padding: '12px 16px 16px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '10px 4px' }}>
                  {cosponsors.map(cs => (
                    <Link
                      key={cs.member_id}
                ...
                {cosponsorCount > cosponsors.length && (
                  <p style={{ fontSize: 11, color: 'var(--t3)', marginTop: 8 }}>
                    외 {cosponsorCount - cosponsors.length}명 <span style={{ opacity: 0.6 }}>(전직 의원)</span>
                  </p>
                )}
              </div>
            </div>
